...
def fast_rcnn_inference(
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            boxes_per_image, scores_per_image, materials_per_image, colors_per_image, image_shape, score_thresh, nms_thresh, topk_per_image
        )
        for scores_per_image, materials_per_image, colors_per_image, boxes_per_image, image_shape in zip(scores, materials, colors, boxes, image_shapes)
    ]
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.material_score = nn.Linear(input_size, num_material + 1)
        self.color_score = nn.Linear(input_size, num_color + 1)
        ...
        nn.init.normal_(self.color_score.weight, std=0.001)
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            "num_material"          : cfg.MODEL.ROI_HEADS.NUM_MATERIALS,
            "num_color"             : cfg.MODEL.ROI_HEADS.NUM_COLORS,
    ...
    def losses_extra_features(self, predictions, proposals):
        ...
        optimizer_colors = optim.SGD(self.color_score.parameters(), lr=0.00025)

        # in your training loop:
        optimizer_materials.zero_grad()
        optimizer_colors.zero_grad()

        loss_materials.backward(retain_graph=True)
        loss_colors.backward(retain_graph=True)

        optimizer_materials.step()
